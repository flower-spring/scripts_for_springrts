...
local function issue(reclaimersList, shift, groundHeightAtFeaturePos)
	...
	for i=1, #reclaimersList do
		...
		if udefId and groundHeight4 and (constructorsAndNanosOfMyPlayerTableAndNecros[uid] and constructorsAndNanosOfMyPlayerTableAndNecros[uid].maxWaterDepth and constructorsAndNanosOfMyPlayerTableAndNecros[uid].minWaterDepth and ((((-(constructorsAndNanosOfMyPlayerTableAndNecros[uid].maxWaterDepth)<=groundHeight4) and (groundHeight4<abs(constructorsAndNanosOfMyPlayerTableAndNecros[uid].minWaterDepth))) or isHovercraft[udefId]))) or canFly[udefId] then
			...
			if len==4 then
				...
				elseif (ct=="ground" and CtrlOrNot) or (ct=="feature" and CtrlOrNot) then
					...
					for k, v in pairs(featuresInSphereFromGround) do
						...
						local translatedDescription=FeatureDefs[featureDefID].translatedDescription
						local tooltip=FeatureDefs[featureDefID].name
						local wreck_in_tooltip_name=false
						local heap_or_debris_in_tooltip_name=false
						if string.find(tooltip, 'Wreck', nil, true) or string.find(translatedDescription, 'Wreck', nil, true) then
							wreck_in_tooltip_name=true
						end
						if (string.find(tooltip, 'Heap', nil, true) or string.find(tooltip, 'Debris', nil, true)) or (string.find(translatedDescription, 'Heap', nil, true) or string.find(translatedDescription, 'Debris', nil, true)) then
